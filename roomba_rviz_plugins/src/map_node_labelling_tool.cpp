...
namespace roomba_rviz_plugins {
    MapNodeLabellingTool::MapNodeLabellingTool() : rviz_common::Tool(), _lastSaved(true), _currentFloor("1"), _markerCount(0) {
        shortcut_key_ = 'n';
    ...
    void MapNodeLabellingTool::onInitialize() {
        ...
        _connectionsMarkerPub = raw_node->create_publisher<visualization_msgs::msg::MarkerArray>("/visualization_marker_array", 10);


        _clearNodesService = raw_node->create_service<std_srvs::srv::Empty>("clearNodes", std::bind(&MapNodeLabellingTool::clearNodes,
                                                                                                      this,
                                                                                                      std::placeholders::_1,
                                                                                                      std::placeholders::_2));

        _loadNodesService = raw_node->create_service<roomba_msgs::srv::LoadConfig>("loadNodes", std::bind(&MapNodeLabellingTool::loadNodes,
                                                                                                            this,
                                                                                                            std::placeholders::_1,
                                                                                                            std::placeholders::_2));

        _addMapNodeService = raw_node->create_service<roomba_msgs::srv::AddMapNode>("addMapNode", std::bind(&MapNodeLabellingTool::addMapNode,
                                                                                                           this,
                                                                                                           std::placeholders::_1,
                                                                                                           std::placeholders::_2));
    }
    ...
    std::shared_ptr<rviz_rendering::Shape> MapNodeLabellingTool::createCircle(double x, double y){
        auto circle = std::make_shared<rviz_rendering::Shape>(rviz_rendering::Shape::Type::Sphere, scene_manager_, nullptr);
