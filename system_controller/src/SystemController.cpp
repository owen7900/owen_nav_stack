...
SystemController::SystemController(const std::string& name)
    : rclcpp::Node(name) {
  this->_cmdVelPub =
      this->create_publisher<geometry_msgs::msg::Twist>("/roomba/cmd_vel", 10);
  ...
  this->_elevatorSub = this->create_subscription<geometry_msgs::msg::Twist>(
      "/elevator_traverser/cmd_vel", 10,
      std::bind(&SystemController::elevatorCallback, this,
                std::placeholders::_1));

  this->_manualSub = this->create_subscription<geometry_msgs::msg::Twist>(
      "/manual/cmd_vel", 10,
      std::bind(&SystemController::manualCmdVelCallback, this, _1));

  this->_bumperSub = this->create_subscription<create_msgs::msg::Bumper>(
      "/roomba/bumper", 10,
      std::bind(&SystemController::bumperCallback, this, _1));

  this->_cliffSub = this->create_subscription<create_msgs::msg::Cliff>(
      "/roomba/cliff", 10,
      std::bind(&SystemController::cliffCallback, this, _1));
  ...
  this->_autonomousCmd.SetTimeout(
      rclcpp::Duration::from_seconds(autonomousTimeout));
  ...
  this->_timer = this->create_wall_timer(
      std::chrono::duration<double>(controlPeriod),
      std::bind(&SystemController::controlCallback, this));
}

void SystemController::bumperCallback(
    const create_msgs::msg::Bumper::ConstSharedPtr msg) {
  auto status = this->_status.GetData();
  status.bumper = *msg;
  this->_status.SetData(status);
}

void SystemController::cliffCallback(
    const create_msgs::msg::Cliff::ConstSharedPtr msg) {
  auto status = this->_status.GetData();
  status.cliff = *msg;
  this->_status.SetData(status);
}
...
bool isWarningZone(const RobotStatus& status) {
  ...
  return bumper.light_signal_center_left > WarningIntensity ||
         bumper.light_signal_front_left > WarningIntensity ||
         bumper.light_signal_left > WarningIntensity ||
         bumper.light_signal_center_right > WarningIntensity ||
         bumper.light_signal_front_right > WarningIntensity ||
         bumper.light_signal_right > WarningIntensity;
}
