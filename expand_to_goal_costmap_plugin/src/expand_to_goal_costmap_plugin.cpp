...
void ExpandToGoal::onInitialize()
{
  ...
  RCLCPP_INFO(logger_, "Subscribing to the map topic (%s) with %s durability", map_topic_.c_str(),
  ...
  map_sub_ = node->create_subscription<nav_msgs::msg::OccupancyGrid>(
      map_topic_, map_qos, std::bind(&ExpandToGoal::incomingMap, this, std::placeholders::_1));
  ...
  this->goalSubscriber = node->create_subscription<geometry_msgs::msg::PoseStamped>(
      "/goal_pose", 10, std::bind(&ExpandToGoal::goalCallback, this, _1));
  this->hasData = false;
  this->current_ = true;
}

void ExpandToGoal::activate()
{
}

void ExpandToGoal::deactivate()
{
}

void ExpandToGoal::reset()
{
...
void ExpandToGoal::getParameters()
{
...
void ExpandToGoal::processMap(const nav_msgs::msg::OccupancyGrid& new_map)
{
  ...
  RCLCPP_DEBUG(logger_, "ExpandToGoal: Received a %d X %d map at %f m/pix", size_x, size_y, new_map.info.resolution);
      ...
      origin_y_ != new_map.info.origin.position.y)
  {
    ...
    RCLCPP_INFO(logger_, "ExpandToGoal: Resizing static layer to %d X %d at %f m/pix", size_x, size_y,
                new_map.info.resolution);
    resizeMapWithGoal(size_x, size_y, new_map.info.resolution, new_map.info.origin.position.x,
...
void ExpandToGoal::resizeMapWithGoal(unsigned int size_x, unsigned int size_y, double resolution, double origin_x,
                                     double origin_y)
{
...
unsigned char ExpandToGoal::interpretValue(unsigned char value)
{
...
void ExpandToGoal::incomingMap(const nav_msgs::msg::OccupancyGrid::SharedPtr new_map)
{
  std::lock_guard<Costmap2D::mutex_t> guard(*getMutex());
  if (!map_received_)
  {
...
void ExpandToGoal::incomingUpdate(map_msgs::msg::OccupancyGridUpdate::ConstSharedPtr update)
{
  std::lock_guard<Costmap2D::mutex_t> guard(*getMutex());
  if (update->y < static_cast<int32_t>(y_) || y_ + height_ < update->y + update->height ||
...
void ExpandToGoal::goalCallback(geometry_msgs::msg::PoseStamped::UniquePtr msg)
{
  if (msg)
  {
    ...
    else
    {
      ...
      RCLCPP_INFO_STREAM(this->logger_, "ExpandToGoal resizing map to " << newSizeX << ", " << newSizeY
                                                                        << " with origin " << newOriginX << ", "
                                                                        << newOriginY);
    }
...
void ExpandToGoal::updateCosts(nav2_costmap_2d::Costmap2D& master_grid, int min_i, int min_j, int max_i, int max_j)
{
  if (!enabled_)
  {
    update_in_progress_.store(false);
